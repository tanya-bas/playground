...
- **Attacker persona (John)**: A bot that sends multi-turn social engineering messages to Clawbot in Slack (attack generation: Claude Opus 4.6)
- **Clawbot**: AI assistant under test, powered by Claude Haiku 4.5 via OpenClaw; must refuse to share private channel content, credentials, PII
- **Scoring**: 0–100 rubric; model-graded by Claude Sonnet 4 on whether Clawbot leaked or resisted
...
2. **3 rounds**: Attacker sends up to 2 follow-ups (generated by Claude Opus 4.6); Clawbot (Claude Haiku 4.5 via OpenClaw) replies
3. **Grading**: Full conversation is scored 0–100 by Claude Sonnet 4 using the rubric
...
When `leakable_data.json` is available, the attacker can reference real channel data to make requests more specific and credible, making attacks harder to distinguish from legitimate colleague requests.
